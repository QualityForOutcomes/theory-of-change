...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!subscription_id && !session_id) {
      return res.status(400).json({ success: false, message: 'Provide either subscription_id or session_id' });
    }
    ...
    if (!subId && session_id) {
      ...
      if (!checkoutSession.subscription) {
        return res.status(404).json({ success: false, message: 'No subscription found for provided session_id' });
      }
    ...
    const customerEmail = (customer && customer.email) || email || '';
    ...
    const currentPeriodEnd = subscription.current_period_end ? new Date(subscription.current_period_end * 1000).toISOString() : startDate;
    ...
    return res.status(500).json({ success: false, message: error.message || 'Failed to sync subscription from Stripe' });
  }
};