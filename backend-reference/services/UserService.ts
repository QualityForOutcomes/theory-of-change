...
async function recordUserPreferences(
    ...
    if (newsLetterSubs) {
        try {
            ...
            try {
                const profile = await getSupabaseUserProfile(normalizedEmail);
                const displayName = profile?.displayName;
                await NewsletterEmailService.sendWelcomeEmail(normalizedEmail, displayName);
                console.log('Newsletter welcome email sent');
            } catch (emailError: unknown) {
                // Do not block registration on email errors
                console.warn('Failed to send newsletter welcome email:', emailError);
            }
        ...
        if (newsLetterSubs === true) {
            ...
            try {
                ...
                try {
                    ...
                    // Do not fail preference update on email errors
                    console.warn('Failed to send newsletter welcome email:', emailError);
                }
            } catch (error: unknown) {
